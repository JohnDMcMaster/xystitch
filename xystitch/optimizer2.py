...
# this package is more standard now
...
def compute_u_std(icm, pairs, xbase, xorder, ybase, yorder):
...
def remove_u_std(icm, pairs, stdev, xbase, xorder, ybase, yorder, x_u, x_sd,
                 y_u, y_sd):
    '''
...
"""
Newer algorithm using uscan.json metadata
Projects points using microscope calibration to estimate where they should be
Points are rejected if they aren't within given statistical deviation
Focus is on feeding data from uscan.json over manuallly specifying parameters

Additionally tests have shown that serpentine is significantly less accurate than
always going from the same direction
As such serpentine support has been removed for now to simplify
Was: r_orders => how many row models
Can add back in the future if required

Assumes always going left/right or right/left

Long term we might try to auto rotate the image as well
this should be possible extracting xy-3p points
Not to be confused with the camera rotation offset
"""
...
class XYOptimizer2:
    def __init__(self, project):
        ...
        self.icm = None
        """
        Reject control points within an image pair that are this many standard deviations
        from other control points within the image pair
        """
        self.image_pair_reject_stdev = 3.0
        # individual control point distances against global set
        self.image_cp_reject_stdev = 3.0
        # When a bad control point is found, is it deleted or just ignored?
        # always delete
        # self.image_pair_reject_delete = True
        self.cp_delete = True
        """
        Estimate control points based on image coordinates
        If they are more than this many standard deviations from stage characterized variation,
        reject them

        Ex: if mean accuracy is 2 um with standard deviation of 1 and a threshold of 3,
        reject any control points that deviate more than 5 um from estimated position
        """
        self.coordinate_reject_stdev = 3.0
        # always delete
        # self.coordinate_reject_delete = True
        # How many types of rows there are
        # traditionally a value of 2 was used to reflect serpintine
        # Now all rows should be the same
        # self.r_orders = 1
        # TODO: some check parameter on camera rotation
        # We are fed an estimate but may come up with something better

    def verify_image_assumptions(self):
        """
        Verify:
        -All images are valid
        -All images are teh same size
        -No fancy stitch parameters
    ...
    def image_neighbor_cp_distance(self, l_il, r_il):
        '''
        FIXME 2024-03-17
        This should be median?
        I think this is causing outlier optimization issues
        ...
        cps_x = []
        cps_y = []
        for cpl in self.cpl_index.get((l_ili, r_il), []):
            # compute distance
        ...
        for cpl in self.cpl_index.get((r_il, l_ili), []):
            cps_x.append(cpl.getv('X') - cpl.getv('x'))
            cps_y.append(cpl.getv('Y') - cpl.getv('y'))
        ...
        if len(cps_x) == 0:
            ...
            # Ideally also through out the adjacent coordinate if the other is bad
            # return (1.0 * sum(cps_x) / len(cps_x), 1.0 * sum(cps_y) / len(cps_y))
            return (statistics.median(cps_x), statistics.median(cps_y))

    def image_neighbors_cp_distance(self):
        """
        Returns
        pairsx[(x, y)]: distance from image(x - 1 , y) to image(x, y)
        pairsy[(x, y)]: distance from image(x , y - 1) to image(x, y)

        NOTE: this is the most computationally expensive code in the optimizer
        We could signifiacntly improve this by writing it in a "one pass" index over the pairs
        """
        pairsx = {}
        pairsy = {}
        # start with simple algorithm where we just sweep left/right
        for y in range(0, self.icm.height()):
            print('Calc delta with Y %d / %d' % (y + 1, self.icm.height()))
            for x in range(0, self.icm.width()):
                img = self.icm.get_image(x, y)
                # Skip missing images
                if img is None:
                    continue
                il = self.project.img_fn2il[img]
                ili = il.get_index()
                if x > 0:
                    img = self.icm.get_image(x - 1, y)
                    if img:
                        pairsx[(x, y)] = self.image_neighbor_cp_distance(
                            self.project.img_fn2il[img], ili)
                    # This is an error / missing data
                    # Favor explicting marking None over no key
                    else:
                        pairsx[(x, y)] = None
                if y > 0:
                    img = self.icm.get_image(x, y - 1)
                    if img:
                        pairsy[(x, y)] = self.image_neighbor_cp_distance(
                            self.project.img_fn2il[img], ili)
                    # This is an error / missing data
                    # Favor explicting marking None over no key
                    else:
    ...
    def check_pair_outlier_overlap(self, icm, pairsx, pairsy):
        """
    ...
    def index_cpls(self, cpls=None):
        """
        ret[(n, N)] = [list of cpls]
        """
        if cpls is None:
            cpls = self.project.control_point_lines
        ret = {}
        for cpl in cpls:
            ret.setdefault((cpl.getv('n'), cpl.getv('N')), []).append(cpl)
        self.cpl_index = ret

    def write_cp_index(self):
        old = len(self.project.control_point_lines)
        self.project.control_point_lines = []
        for cpls in self.cpl_index.values():
            self.project.control_point_lines.extend(cpls)
        print("Updating cp index: %u => %u" %
              (old, len(self.project.control_point_lines)))

    def print_check_control_points(self):
        fail = False
        counts = {}
        for cpl in self.project.get_control_point_lines():
            n = cpl.getv('n')
    ...
    def print_pairsxy(self, pairsx, pairsy):
        print('Delta map')
        for y in range(0, self.icm.height()):
            for x in range(0, self.icm.width()):
                print('  %03dX, %03dY' % (x, y))
    ...
    def check_pair_outlier_u_std(self, icm, pairs, xbase, xorder, ybase,
                                 yorder):
        """
        xorder, yorder: how many rows/cols to group together
        """

        stdev = self.image_pair_reject_stdev
        print((
            'Checking for outliers by u/sd, x=range(%u, w+1, %u), y=range(%u, h+1, %u)'
            % (xbase, xorder, ybase, yorder)))
        val = compute_u_std(icm, pairs, xbase, xorder, ybase, yorder)
        if not stdev or val is None:
            print('stdev filter: none')
        else:
            x_u, x_sd, y_u, y_sd = val
            print(('stdev filter: %0.3f' % stdev))
            remove_u_std(icm, pairs, stdev, xbase, xorder, ybase, yorder, x_u,
                         x_sd, y_u, y_sd)

    def imagen_to_cr(self, n):
        il = self.project.image_lines[n]
        name = il.get_name()
        return self.icm.fn2cr[name]

    def iter_cpl_deltas(self):
        """
        Try to return positive values
        The image to the right will have coordinates at left side
        This means we generally want to subtract left.x - right.x
        """
        for (im_n, im_N), cpls in self.cpl_index.items():
            for cpl in cpls:
                im_n_c, im_n_r = self.imagen_to_cr(im_n)
                im_N_c, im_N_r = self.imagen_to_cr(im_N)
                # default: n - N
                dx = cpl.getv('x') - cpl.getv('X')
                dy = cpl.getv('y') - cpl.getv('Y')

                # Need to normalize to N - n?
                if im_n_c != im_N_c:
                    invert = im_n_c > im_N_c
                elif im_n_r != im_N_r:
                    invert = im_n_r > im_N_r
                else:
                    assert 0
                if invert:
                    yield (cpl, im_N, im_N_c, im_N_r, im_n, im_n_c, im_n_r,
                           -dx, -dy)
                else:
                    yield (cpl, im_n, im_n_c, im_n_r, im_N, im_N_c, im_N_r, dx,
                           dy)

    def clean_control_points_u_std(self):
        ...
        for (cpl, _im_n, im_n_c, im_n_r, _im_N, im_N_c, im_N_r, dx,
             dy) in self.iter_cpl_deltas():
            if im_n_c != im_N_c:
                dx_dc.append(dx)
                dy_dc.append(dy)
            elif im_n_r != im_N_r:
        ...
        print("stdev threshold: %0.3f" % self.image_cp_reject_stdev)

        def filter_points(x_u, x_std, y_u, y_std, selector):
            removed = 0
            cpls_new = []
            for iter_this in self.iter_cpl_deltas():
                (cpl, _im_n, im_n_c, im_n_r, _im_N, im_N_c, im_N_r, dx,
                 dy) = iter_this
                # One we should inspect?
                if not selector(im_n_c - im_N_c, im_n_r - im_N_r):
                    cpls_new.append(cpl)
                    continue

                def in_bounds(u, std, value):
                    return u - self.image_cp_reject_stdev * std <= value <= u + self.image_cp_reject_stdev * std

                dx = cpl.getv('x') - cpl.getv('X')
                dy = cpl.getv('y') - cpl.getv('Y')
                if in_bounds(x_u, x_std, dx) and in_bounds(y_u, y_std, dy):
                    cpls_new.append(cpl)
                else:
                    removed += 1
                    print("Removing CP w/ delta dx=%0.3f dy=%0.3f" % (dx, dy))
            print("Removed %u / %u points => %u remain" %
                  (removed, removed + len(cpls_new), len(cpls_new)))
            self.index_cpls(cpls=cpls_new)
        ...
        if len(dx_dc) >= 2:
            ...
            dx_dc_u = statistics.mean(dx_dc)
            dx_dc_std = statistics.stdev(dx_dc)
            dy_dc_u = statistics.mean(dy_dc)
            dy_dc_std = statistics.stdev(dy_dc)
            print("dx/dc u=%0.3f, std=%0.3f" % (dx_dc_u, dx_dc_std))
            print("dy/dc u=%0.3f, std=%0.3f" % (dy_dc_u, dy_dc_std))
            filter_points(dx_dc_u, dx_dc_std, dy_dc_u, dy_dc_std, selector)
        ...
        if len(dx_dr) >= 2:
            ...
            dx_dr_u = statistics.mean(dx_dr)
            dx_dr_std = statistics.stdev(dx_dr)
            dy_dr_u = statistics.mean(dy_dr)
            dy_dr_std = statistics.stdev(dy_dr)
            print("dx/dr u=%0.3f, std=%0.3f" % (dx_dr_u, dx_dr_std))
            print("dy/dr u=%0.3f, std=%0.3f" % (dy_dr_u, dy_dr_std))
            filter_points(dx_dr_u, dx_dr_std, dy_dr_u, dy_dr_std, selector)
    ...
    def clean_control_points(self):
        ...
        # if self.verbose:
        #    self.print_check_control_points()

        # All control points should be roughly the same translation
        # This should fix most issues
        for i in range(2):
            self.clean_control_points_u_std()
        ...
        pairsx, pairsy = self.image_neighbors_cp_distance()

        if self.verbose:
            self.print_pairsxy(pairsx, pairsy)

        print("")
        # Remove anything that is grossly outside of expected overlap
        # TODO: replace this with more advanced calibration / coordinate check
        self.check_pair_outlier_overlap(self.icm, pairsx, pairsy)
        """
        Today serpentine pattern left/rightprint_pairsxy
        ...
        """
        # self.image_pair_reject_stdev
        self.check_pair_outlier_u_std(self.icm,
                                      pairsy,
                                      xbase=0,
                                      xorder=1,
                                      ybase=0,
    ...
    def xy_opt(self):
        ...
        print("")
        print("Control points cleaned. Computing fresh position estimates")
        pairsx, pairsy = self.image_neighbors_cp_distance()

        # Chose an image in the center to attach other images to
        anch_c, anch_r = anchor(project, icm, use_cr=self.anchor_cr)
    ...
    def run(self, anchor_cr=None, check_poor_opt=True):
        ...
        bench = Benchmark()
        self.anchor_cr = anchor_cr
        self.should_check_poor_opt = check_poor_opt
        ...
        fns = []
        # Copy project so we can trash it
        self.project, self.project_orig = self.project.copy(), self.project
